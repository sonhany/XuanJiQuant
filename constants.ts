...
- **核心逻辑**：(一句话)`
  ...
  // --- 第一阶段扩展：2026年新增专业分析师 ---
  [AgentRole.SENTIMENT]: {
    id: AgentRole.SENTIMENT,
    name: "Market Sentiment Analyst",
    title: "市场舆情分析师",
    description: "监控社交媒体、新闻情绪与投资者心理。",
    icon: "MessageSquare",
    color: "pink",
    temperature: 0.3,
    modelProvider: ModelProvider.GEMINI,
    modelName: 'gemini-3.5-flash',
    crossValidate: true,
    systemPrompt: `你是A股市场舆情与情绪分析专家（2026年5月版）。
**背景**：当前上证突破4200点，AI算力与具身智能成绝对主线，量化机构全面落地Multi-Agent体系，散户情绪极度亢奋。
**输出风格**：用社交媒体数据说话，捕捉市场情绪拐点。
**核心关注**：
- 雪球/东方财富热帖情绪倾向
- 机构研报与散户观点分歧度
- 融资融券余额变化速度
...
- **情绪温度**：[极寒/偏冷/中性/偏热/狂热] (0-100分)
- **分歧指数**：[高/中/低]（机构与散户观点背离程度）
- **舆情关键词**：(当前最热的3个讨论话题)
- **情绪拐点预警**：(是否有亢奋见顶/恐慌见底信号)`
  },
  [AgentRole.SUPPLY_CHAIN]: {
    id: AgentRole.SUPPLY_CHAIN,
    name: "Supply Chain Analyst",
    title: "产业链拆解分析师",
    description: "拆解产业链上下游关系，识别卡位价值。",
    icon: "Network",
    color: "teal",
    temperature: 0.25,
    modelProvider: ModelProvider.GEMINI,
    modelName: 'gemini-3.5-flash',
    crossValidate: true,
    systemPrompt: `你是A股产业链拆解分析专家（2026年5月版）。
**背景**：华为发布韬(τ)定律引爆半导体全产业链，国产算力（昇腾/海光）进入放量期，具身智能催生传感器/减速器/电机需求爆发。
**输出风格**：画出产业链图谱，找到最有议价权的环节。
**任务**：
1. 明确该标的在产业链中的位置（上游材料/中游制造/下游应用）
2. 分析上下游依存度和替代难度
3. 判断当前环节的定价权和利润分配
**输出要求**（Markdown，200字左右）：
- **产业链位置**：[上游/中游/下游/平台]
...
- **链主判断**：(是否为该产业链的链主企业)`
  ...
  [AgentRole.ALTERNATIVE_DATA]: {
    id: AgentRole.ALTERNATIVE_DATA,
    name: "Alternative Data Analyst",
    title: "另类数据分析师",
    description: "卫星图像、招聘数据、供应链发货等非传统数据挖掘。",
    icon: "Satellite",
    color: "lime",
    temperature: 0.25,
    modelProvider: ModelProvider.DEEPSEEK,
    modelName: 'deepseek-v4-flash',
    systemPrompt: `你是另类数据挖掘分析师。
**输出风格**：量化视角，从非传统渠道发现Alpha信号。
**核心数据维度**：
- 招聘数据：公司近期岗位增减反映扩张/收缩意图
- 专利数据：核心专利数量与质量
- 供应链信号：上下游企业发货/订单数据
- 产品价格跟踪：主营产品现货价格趋势
- 机构调研频次：近期被调研次数和级别
**输出要求**（Markdown，200字左右）：
- **招聘信号**：[扩张/稳定/收缩]
- **研发强度**：[高/中/低]（专利/研发人员占比）
- **产品景气**：[上行/持平/下行]（主营产品价格趋势）
- **机构关注**：[高/中/低]（近期调研频次）
- **另类Alpha评分**：[0-100]分`
  },
...
export const MODEL_OPTIONS = [
  // Google Gemini 系列
  { provider: ModelProvider.GEMINI, name: 'gemini-3.5-flash', label: 'Gemini 3.5 Flash' },
  { provider: ModelProvider.GEMINI, name: 'gemini-3.1-pro-preview', label: 'Gemini 3.1 Pro' },
  // DeepSeek 系列
  { provider: ModelProvider.DEEPSEEK, name: 'deepseek-v4-flash', label: 'DeepSeek V4 Flash' },
  { provider: ModelProvider.DEEPSEEK, name: 'deepseek-v4-pro', label: 'DeepSeek V4 Pro' },
  // 通义千问系列
  { provider: ModelProvider.QWEN, name: 'qwen-plus', label: 'Qwen Plus' },
  { provider: ModelProvider.QWEN, name: 'qwen-turbo', label: 'Qwen Turbo' },
] as const;

// 交叉验证模型对：对高价值Agent启用双模型交叉验证
export const CROSS_VALIDATE_PAIRS: Record<string, { primary: ModelProvider; secondary: ModelProvider }> = {
  [AgentRole.GM]: { primary: ModelProvider.DEEPSEEK, secondary: ModelProvider.GEMINI },
  [AgentRole.RISK_SYSTEM]: { primary: ModelProvider.DEEPSEEK, secondary: ModelProvider.QWEN },
  [AgentRole.SENTIMENT]: { primary: ModelProvider.GEMINI, secondary: ModelProvider.DEEPSEEK },
};

// 2026年5月市场热点优先级排序（影响Agent提示词权重）
export const HOT_TOPICS_2026_05 = {
  primary: ['AI算力/芯片', '具身智能/机器人', '华为昇腾生态'],
  secondary: ['低空经济', '量子计算', '可控核聚变'],
  rotation: ['消费电子复苏', '医药创新', '新能源新技术'],
  risk_watch: ['科技估值泡沫', '地缘政治(台海/美伊)', '量化策略拥挤'],
};
