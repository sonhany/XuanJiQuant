...
前提: 后端运行在 localhost:8880，数据已 seed
"""
import json
import os
import sys
...
def _load_token():
    if os.environ.get("XUANJI_API_TOKEN"):
        return os.environ["XUANJI_API_TOKEN"]
...
def call(method, path, body=None, note=""):
    ...
    try:
        if method == "POST":
            ...
            if API_TOKEN:
                headers["X-XuanJi-Token"] = API_TOKEN
...
def run_case(name, method, path, body=None, validate=None, critical=True):
    """执行单个测试用例。返回 (ok, data)"""
...
def main():
    ...
    ap.add_argument("--destructive", action="store_true", help="允许下单、成交、重置账户和修改告警；默认只读")
    ap.add_argument("--reset", action="store_true", help="测试前重置模拟账户；必须同时指定 --destructive")
    args = ap.parse_args()
    if args.reset and not args.destructive:
        ap.error("--reset requires --destructive")
    ...
    run_case("GET /api/market/indices", "GET", "/api/market/indices", validate=v_indices)
    ...
    run_case("POST stocks (市场浏览)", "POST", "/api/data",
    ...
    run_case("POST meta (因子列表)", "POST", "/api/factor", {"action": "meta"}, validate=v_fmeta)
    ...
    run_case("POST evaluate (单因子IC)", "POST", "/api/factor",
    ...
    run_case("POST evaluate_all (批量IC)", "POST", "/api/factor",
    ...
    run_case("POST all (总览刷新)", "POST", "/api/execution", {"action": "all"}, validate=v_all)
    ...
    if args.destructive:
        ok_place, place_data = run_case("POST place_order (买入)", "POST", "/api/execution",
             {"action": "place_order", "code": "000001", "direction": "buy", "quantity": 100, "order_type": "limit", "price": 10.6},
             validate=v_place)

        if ok_place:
            d = place_data.get("data", {})
            oid = d.get("id") or (d.get("order", {}) or {}).get("id")
            run_case("POST fill_order (成交)", "POST", "/api/execution",
                 {"action": "fill_order", "order_id": oid, "fill_price": 10.6},
                 critical=False)
    else:
        print("  ⏭️  place_order/fill_order 跳过（默认只读）")
    ...
    run_case("POST stats", "POST", "/api/alerts", {"action": "stats"}, validate=v_stats)

    def v_list(d):
        data = d.get("data", {})
        assert "alerts" in data, f"list no alerts: {list(data.keys())}"
    run_case("POST list", "POST", "/api/alerts", {"action": "list", "limit": 50}, validate=v_list)

    run_case("POST list (status=active)", "POST", "/api/alerts",
    ...
    run_case("POST rules", "POST", "/api/alerts", {"action": "rules"}, validate=v_rules)

    if args.destructive:
        run_case("POST check (扫描告警)", "POST", "/api/alerts", {"action": "check"}, critical=False)
    else:
        print("  ⏭️  alerts.check 跳过（默认只读）")
    ...
    if args.destructive and rules_list:
        rid = rules_list[0].get("id")
        run_case("POST update_rule (切换规则)", "POST", "/api/alerts",
             {"action": "update_rule", "id": rid, "enabled": True}, critical=False)
        run_case("POST silence (静默)", "POST", "/api/alerts",
    ...
    if args.destructive and alerts:
        aid = alerts[0].get("id")
        run_case("POST acknowledge (确认)", "POST", "/api/alerts",
             {"action": "acknowledge", "alert_id": aid}, critical=False)
        run_case("POST resolve (解决)", "POST", "/api/alerts",
